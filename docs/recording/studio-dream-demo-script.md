...
| 3 | `/studio?demo=coast` | Point at `Bridge QA 证据状态`. | The card shows whether bridge assets exist, e.g. `1 个桥接素材` and `Studio-Dream 闭环已验证`. |
| 4 | `/studio?demo=coast` | Point at `Demo Bridge`. | The card says `三亚海岛 → Dream`, shows `Recording suite 已覆盖`, and links to the matching Dream preview. |
| 5 | `/dream?demo=coast` | Click `打开同款梦境预览`. | Dream opens directly into the coastal route, dusk mood, and starlake template. |
| 6 | `/dream?demo=coast` | Point at `Studio Bridge`. | The visual preview explains how to return to the matching recording workbench. |
| 7 | `/studio?demo=coast` | Click `返回同款录屏台`. | Studio returns to the same 三亚海岛 demo instead of resetting to Dali. |

## Voiceover Cues

- "我把录屏台和梦境路书接成一条演示路径。"
- "Studio 负责讲 Agent 工作流，Dream 负责展示用户真正看到的高级路书画面。"
- "Bridge QA 状态卡告诉我，这条 Studio-Dream 闭环已经有截图证据。"
...
- Use Shot 1-4 as the process/tutorial part.
- Use Shot 5 as the product visual part.
- Use Shot 6-7 as the "Agent workflow is becoming a real product" transition.
