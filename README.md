...
`/studio` reads `/api/recording-assets` and shows the current local recording asset count, latest QA pack, recent QA packs with Dream/Studio badges and usage hints, a refresh control, and an "打开总索引" link. The link opens `/api/recording-assets/index`, a local HTML overview with the same pack type and usage labels. If the local index is missing, `/studio` shows the exact command: `npm run check:recording-suite`.
