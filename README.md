...
`npm run check:studio-visuals` expects `http://localhost:3000/studio` unless `STUDIO_URL` is set. It captures the Dali and coastal 16:9 recording layouts, verifies the Studio proof playback, captures the Proof Story `脚本素材` card, verifies the Proof Story Handoff copy state, records the Proof Story Complete strip, and writes `summary.json`, `index.html`, and `clip-notes.md` under `recordings/studio-checks/`.
