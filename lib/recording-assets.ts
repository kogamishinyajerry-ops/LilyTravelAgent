...
async function readLatestRecordingIndexCheck(recordingsRoot: string): Promise<RecordingIndexCheckSummary | null> {
  ...
  for (const entry of entries) {
    ...
    const proofChainSummaryLine = await readProofChainSummaryLine(notesPath ? notesFilePath : "", summary);
...
async function readProofChainSummaryLine(notesPath: string, summary: Record<string, unknown>) {
  const notes = notesPath ? await readFile(notesPath, "utf8").catch(() => "") : "";
  const notesLine = readProofStoryLine(notes, "Proof Chain Summary ·");
  if (notesLine) {
    return notesLine;
  }
  ...
  const scriptMaterialCheck = typeof summary.scriptMaterialCheck === "object" && summary.scriptMaterialCheck
    ? summary.scriptMaterialCheck as Record<string, unknown>
    : null;
  const proofTextLine = readProofStoryLine(readString(scriptMaterialCheck?.proofText), "Proof Chain Summary ·");
  if (proofTextLine) {
    return proofTextLine;
  }
  ...
  return readString(scriptMaterial?.proofChainSummaryLine);
}
...
function readStudioScriptMaterialFromSummary(entry: string, summary: Record<string, unknown>): RecordingStudioScriptMaterialSummary | null {
  ...
  const proofChainSummaryLine = readString(scriptMaterial.proofChainSummaryLine);
  const proofChainSummaryCopyState = readString(scriptMaterial.proofChainSummaryCopyState);
  return {
    ...
    ...(proofChainSummaryLine ? { proofChainSummaryLine } : {}),
    ...(proofChainSummaryCopyState ? { proofChainSummaryCopyState } : {}),
    screenshotPath: screenshotFile ? toRecordingLink(path.join("studio-checks", entry, screenshotFile)) : "",
  };
}
